...
const ifrWxPattern =
  /\b([0-2](\s[0-9]\/[0-9])?|[0-9]\/[0-9])SM(\sR\d{2}\/P?\d{1,4}([VP]+\d{1,4})FT\/[DNU])*(\s([-+])?((PR|BL|SH|TS|FZ|VC|)?((RA|SN|DZ){1,3}|(DZ|RA|SN|SG|IC|PE|PL|GR|GS|FC|BR|FG|FZFG|HZ|FU|VA|DU|SA|PY))(\s([-+])?(BL|SH|TS|FZ|VC|)?((RA|SN|DZ){1,3}|(DZ|RA|SN|SG|IC|PE|PL|GR|GS|FC|BR|FG|FZFG|HZ|FU|VA|DU|SA|PY)))*))?\b/g;
