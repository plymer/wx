# Readme

## To Do:
- [x] TAF Plus [JSON](https://metaviation.ec.gc.ca/hubwx/scripts/getForecasterNotes.php?site0=CYYZ&site1=CYUL&site2=CYYC&site3=CYVR)
- [x] Fix METAR/metadata/TAF duplication
- [x] Fix background of mode buttons not showing on first load
- [ ] Enable NOTAM lookup and mode switching
- [ ] PIREPs, SIGMETs, AIRMETs (using [dd.weather.gc.ca](https://dd.weather.gc.ca/cgi-bin/bulletin_search.pl?product=ws,wc,wv&header=21,22,23,24,25,26,27&location=cn) or [CFPS](https://plan.navcanada.ca/weather/api/alpha/?site=CZEG&site=CZVR&site=CZWG&site=CZYZ&site=CZUL&site=CZQM&site=CZQX&alpha=sigmet&alpha=airmet))
- [ ] extra product charts (HLTs, SIGWXs, etc)
- [x] thunderstorm outlooks
- [x] set up basic public forecast retrieval and UI
- [x] create map target for outlooks/warnings
