import { datetime, float, int, mysqlEnum, mysqlTable, primaryKey, smallint, text, varchar, } from "drizzle-orm/mysql-core";
...
export const metars = mysqlTable("metars", {
...
export const tafs = mysqlTable("tafs", {
    siteId: varchar("siteId", { length: 4 }).notNull(),
    validTime: datetime("validTime").notNull(),
    rawText: text("rawText"),
}, (table) => [primaryKey({ columns: [table.siteId, table.validTime] })]);
export const pireps = mysqlTable("pireps", {
    validTime: datetime("validTime").notNull(),
    lat: float("lat").notNull(),
    lon: float("lon").notNull(),
...
export const sigmets = mysqlTable("sigmets", {
