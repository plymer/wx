...
import { DEFAULT_REMOTE_HEADERS, MINUTE } from "../lib/constants.js";
import { createSwrCacheHandler } from "../lib/swrCacheHandler.js";

const lightningSwr = createSwrCacheHandler<Feature<MultiPoint, { validTime: number }>[]>({
  cacheKey: "wxmapMetars",
  freshMs: MINUTE,
  staleMs: 5 * MINUTE,
  fetchFn: fetchLightningData,
});

async function fetchLightningData(): Promise<Feature<MultiPoint, { validTime: number }>[]> {
