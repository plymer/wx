...
export const lightningRouter = router({
  lightning: publicProcedure.query(async () => {
    const cachedData = await cacheClient.get("lightning:latest");

    if (cachedData) {
      console.log(`[API] Cache HIT for lightning data`);
      return JSON.parse(cachedData) as Feature<MultiPoint, { validTime: number }>[];
    }

    console.log(`[API] Cache MISS for lightning data. Fetching from source...`);
    ...
    try {
      ...
      await cacheClient.setEx("lightning:latest", 60 * 5, JSON.stringify(features));
