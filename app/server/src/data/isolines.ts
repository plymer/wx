  ...
  try {
    ...
    for (const type of DATA_TYPES) {
      ...
      const tupleData = Object.values(collatedData).reduce<Tuple2DWithValue[]>((acc, data) => {
        ...
        const computedTime = now - 90 * MINUTE;
      ...
      const res = CONFIG[type].resolution as number | [number, number];

      const fc = tupleArrayToGeoJson(tupleData, {
        resolution: res as [number, number],
        sigma: CONFIG[type].sigma,
        thresholdStep: CONFIG[type].spacing,
        extrema: !!(type === "mslp"),
        extremaOptions: { minProminence: 0.00001, minSeparation: 36 },
      });

      featureCollectionsByType[type] = fc as FeatureCollection<
